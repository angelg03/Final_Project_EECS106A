    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            for dist in np.arange(r,0,-min(self._x_res, self._y_res)):
                ...
                # #determine if it's occupied or free
                if last_voxel == None :
                    last_voxel = True
                    #occupied, grid values are negative, upper bounded by threshold, so we want to take min of thresh and new value
                    self._map[grid_x, grid_y] = min(self._occupied_threshold, self._map[grid_x, grid_y]+self._occupied_update)
                else:
                    #free, lower bounded by threshold
                    self._map[grid_x, grid_y] = max(self._free_threshold, self._map[grid_x, grid_y]+self._free_update)
